...
def main():
    ...
    logging.info(
        "Browser Shafa cycle | start=%s | brands=%s",
        start_index,
        ", ".join(
            spec["name"]
            for spec in batch
    ...
    try:
        for spec in batch:
            ...
            # First browser visit for this brand only memorises
            # the current listings. No TEST messages are sent.
            if state.get_meta(
                baseline_key
            ) != "1":
                for item in items:
            ...
            for item in reversed(items):
                ...
                # Mark first so one irrelevant item is not
                # repeatedly processed in future runs.
                state.mark(
                    item.listing_id
                )
            ...
            # Keep local state current during the scan.
            state.save()

        next_index = (
            start_index + per_cycle
        ) % len(brands)
        ...
        state.set_meta(
            "browser_monitor_v2_active",
            "1",
        )
        state.save()

        logging.info(
            "Cycle complete. Next brand index: %s",
            next_index,
        )
